const cards = [
  ...
  { letter: 'N', colorLetter: 'text-secondary', colorCard: 'border-secondary/30 bg-secondary/[0.06]', name: 'Narrate', desc: 'The agentic design system layer. Domain knowledge as machine-readable context.', bracketed: true },
...
export function INCLoop() {
  return (
    <section className="px-20 py-24 max-w-[1440px] mx-auto bg-surface-deep">
      ...
      <div className="flex items-stretch max-w-[960px] mx-auto gap-0">
        {cards.map((card) => (
          <div key={card.letter} className="flex-1 flex items-stretch">
            {card.bracketed && <span className="font-display text-5xl font-bold text-secondary/60 flex items-center pr-3">&lt;</span>}
            <div className={`flex-1 flex flex-col items-center gap-4 p-8 rounded-xl text-center border ${card.colorCard} ${!card.bracketed ? 'mx-4' : ''}`}>
            ...
            {card.bracketed && <span className="font-display text-5xl font-bold text-secondary/60 flex items-center pl-3">&gt;</span>}
          </div>
        ))}
      </div>
    </section>
